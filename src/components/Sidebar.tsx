...
const Sidebar = () => {
  ...
  useEffect(() => {
    const fetshCategories = async () => {
      try {
        ...
        const data: FetchResponse = await resposne.json();
  ...
  const handleMinPriceChnage = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setMinPrice(value ? parseFloat(value) : undefined);
  };

  const handleMaxPriceChnage = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
  ...
  return (
    <div className="w-64 p-5 h-screen">
      <h1 className="text-2xl font-bold mb-10 mt-4">ShopFusion</h1>
